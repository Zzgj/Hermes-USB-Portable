...
function Move-LegacyHermesSourceState {
    ...
    param(
        ...
        [object]$FallbackSourceState = $null
    )
    ...
    $legacyStatePresent = Test-Path -LiteralPath $legacySourceStatePath -PathType Leaf

    if (-not $canonicalStateValid) {
        if (
            $legacyStatePresent -and
            (Test-HermesSourceStateFile -Path $legacySourceStatePath -ExpectedComponentLockHash $ExpectedComponentLockHash)
        ) {
            $replacementSourceState = Get-Content -LiteralPath $legacySourceStatePath -Raw -Encoding UTF8 | ConvertFrom-Json
        }
        elseif ($null -ne $FallbackSourceState) {
            $replacementSourceState = $FallbackSourceState
        }
        else {
            return $false
        }

        Write-HermesSourceState -RuntimeDirectory $RuntimeDirectory -SourceState $replacementSourceState | Out-Null
        $canonicalStateValid = Test-HermesSourceStateFile -Path $sourceStatePath -ExpectedComponentLockHash $ExpectedComponentLockHash
        if (-not $canonicalStateValid) {
            throw "The replacement Hermes source state is invalid."
        }
    }
