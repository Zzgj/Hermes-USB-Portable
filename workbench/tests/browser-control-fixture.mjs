...
const server=await startControlServer({assets:fileURLToPath(new URL('../dist',import.meta.url)),startInstance:async()=>({state:'ready',connection:Promise.resolve({port:12345,token:'fixture-no-real-backend-token'}),stop:async()=>{}}),prepareLearn:async request=>({kind:'learn',prompt:`P2 synthetic learn prompt: ${request.source}; ${request.scope}`,builderFingerprint:'a'.repeat(64)}),prepareCapability:async({card,values})=>({kind:'capability',cardId:card.id,method:card.method,availability:'unknown',prompt:`Synthetic capability ${card.id}: ${JSON.stringify(values)}`})});
