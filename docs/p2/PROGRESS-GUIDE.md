...
更新：2026-09-20。**开发整合中，非 RC，不能称只差用户验收。**

2026-09-21：本机现场现保存到 `handoff/p2-local-checkpoint-20260921`，未合并 dff159b。
下表“未提交”描述的是保存前状态；原远端开发分支保持 dff159b，不能把任一线当作整合完成。
本文件是 P2 唯一任务状态入口；自动测试、合成浏览器、隔离接口和实机证据分别记录。
总体定位见 [总计划](../project_plan.md)；具体接口查 [工程手册](ENGINEERING.md)。

## 三种状态不要混淆

| 代码线 | 内容 | 验证与问题 |
|---|---|---|
| 本机 HEAD aecd17d | 上一次已提交的交接基线 | 不能代表当前有 WIP 的目录 |
| 本机 WIP（未提交） | 卡片管理接口准备/确认重核、轮次关联、观测报告、任务→学习入口半成品 | 2026-09-20：115 项测试/16 组件通过，生产构建失败两处；未发布 |
| 外部 dff159b | 公司/其他模型提交的 A/B/C/D 实现 | 先前独立 Linux 审查 151 项测试/构建通过；发现六项问题，未整合、未验收 |

本机未提交的 P2-17 与 dff159b 的实现重叠，不能直接拼接，也不能继承另一线的测试通过状态。
远程跟踪引用截至本轮读取得到 dff159b，本轮未重新 fetch。开始开发前必须重新核对。
文档中的“本机负责人审批”是开发质量门，不替代用户对工具/系统操作的授权。

## 先做什么

1. **S0**：按 [HANDOFF](HANDOFF.md) 保护现场、确认 Git 保存/整合方式、建立统一基线。
2. **S1**：修复任务→学习入口构建，解决目录服务/令牌接线及执行前重核，补整合测试。
3. **S2**：完成学习产出关联、可信度/环境边界、受控持久化与数据投影。
4. **S3**：配置/更新/原生入口、会话恢复与真实状态、视觉及无障碍。
5. **S4**：过开发门禁后交付集中包与清单；Windows/U 盘缺环境时明确待验。

不得跳过 S0 直接下发“把 P2 全部做完”。每轮任务单与回交材料格式见 HANDOFF。

## 当前任务清单

以下从原总计划迁入，描述的是本机 WIP 的实现范围；没有因此授信外部交付或宣告完整通过。
P2-17 的代码已含重核逻辑，但其所在完整 WIP 当前构建失败，需整合回归后才可交付。
