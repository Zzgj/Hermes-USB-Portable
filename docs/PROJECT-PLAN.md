...
| 当前任务 | **P0 RC2：开发收尾与集中验收候选包** |
| 下一个验收门 | Windows CI 通过后交付整包；一次完成沙箱升级、入口、诊断与 U 盘迁移验收 |
| 最近更新 | 2026-09-05 |

## 状态说明

2026-09-05 集成状态：`docs/P0-RC1-ACCEPTANCE.md` 为下一轮集中验收入口。已实现已知大小写冲突的校验备份/失败恢复、位置迁移门禁及 venv 实际路径检查、四入口能力检测、宿主落地摘要和整包校验部署。Soft Reset 重建、官方更新及两层回执已有实机证据。历史任务条目中的待验文字按下文与 RC1 说明的最新结果解释；仍不能把 P0 标记完成，U 盘迁移、新入口及完整更新失败回退尚需集中验证。外壳图形自动更新不属于本 RC 已实现能力。
...
| P0-14 交付 | 回归 CI、脱敏报告、备份目录清单；RC2 独立版本名与 docs/P0-RC2-ACCEPTANCE.md | 双 Windows CI 通过后的制品校验与集中验收 |

上述表格不是完成声明。不得把核心探针全绿等同于会话恢复、所有入口
或任意更新失败恢复均通过。当前仍处于 P0，不进入 P1/P2。
