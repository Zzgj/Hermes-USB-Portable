...
| 下一个验收门 | 在已恢复健康的 E 盘脱敏沙箱完成 P0-10 受控下载中断、失败关闭和使用已校验缓存续跑；GitHub Git 通道恢复后再续验 P0-07 官方更新 |
| 最近更新 | 2026-09-05 |
...
- [-] **P0-10** 测试中断重试、损坏缓存、更新失败、软重建和数据保留；已实现按锁指纹保存的原子组件回执、跳过前实时可执行验证、既有成功 Runtime 的回执迁移、无效回执单步重建、Hermes Git 暂存跨运行保留、用户已更新 Hermes 源码保护、启动器 `ready.flag` 后置检查、明确非零失败状态及 Playwright 独立日志。用户 Win10/exFAT 已运行全部七组测试，并将既有健康 Runtime 无重装迁移为 `Ready=True`/10 份回执，重复启动没有重跑 Setup。E 盘脱敏沙箱已完成 40,105 个文件/1.43 GiB 的复制校验和 checkout 清洁化。2026-09-05，该沙箱先后通过损坏 ripgrep 回执的单步重建、Playwright 独立日志，以及损坏 `rg.zip` 后的缓存校验拒绝与重新下载；两次均恢复 `ready.flag`、有效回执、原可执行文件哈希、Git clean 和数据/知识哨兵，且其他回执不变。两份证据分别位于 `p0-ripgrep-recovery-20260905-123146/result.json` 与 `p0-rg-cache-recovery-20260905-124011/result.json`。尚需验证中断续跑与 Soft Reset 数据保留，不开启 Full Reset。
...
1. 继续 **P0-10** 实机验收：checkout 清洁化、损坏 ripgrep 回执单步重建、Playwright 独立日志及损坏 `rg.zip` 的拒绝/重新下载均已通过，沙箱已恢复健康。下一项在该沙箱使用临时不可达 URL 制造受控下载中断，确认非零退出且没有 `ready.flag`，随后恢复原锁和已校验缓存续跑；最后验证 Soft Reset 数据保留，不执行 Full Reset。
