...
| 下一个验收门 | 先在 E 盘源实例和脱敏沙箱执行 Hermes checkout 清洁化，再完成 P0-10 损坏回执单步重建；GitHub Git 通道恢复后再续验 P0-07 官方更新 |
| 最近更新 | 2026-09-04 |
...
- [-] **P0-10** 测试中断重试、损坏缓存、更新失败、软重建和数据保留；已实现按锁指纹保存的原子组件回执、跳过前实时可执行验证、既有成功 Runtime 的回执迁移、无效回执单步重建、Hermes Git 暂存跨运行保留、用户已更新 Hermes 源码保护、启动器 `ready.flag` 后置检查、明确非零失败状态及 Playwright 独立日志。用户 Win10/exFAT 已运行全部七组测试，并将既有健康 Runtime 无重装迁移为 `Ready=True`/10 份回执，重复启动没有重跑 Setup。E 盘脱敏沙箱已完成 40,105 个文件/1.43 GiB 的数量、总大小和抽样 SHA-256 校验，且确认未复制私人数据；其中的两项 Git dirty 状态也与源实例精确一致，须先应用 P0-07/P0-12 清洁化修复再执行故障注入。尚需在该副本验证中断/损坏/软重建和 Playwright 日志，不开启 Full Reset。
...
1. 继续 **P0-10** 实机验收：E 盘脱敏沙箱已完成数量/大小/抽样哈希校验。先在源实例和沙箱运行新 Setup 的 checkout 清洁化迁移并确认 `git status` 为 clean，再执行损坏 ripgrep 回执的单步重建、Playwright 独立日志和 Soft Reset 数据保留，不执行 Full Reset。
