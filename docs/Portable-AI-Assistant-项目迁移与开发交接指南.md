...
- P0-07 本地模拟已覆盖官方成功、未审计 origin 拒绝和官方失败退出；GitHub Actions run `33783429258` 的 Windows PowerShell 5.1/7 七组测试全部通过。`docs/p0/ACCEPTANCE.md` 是当前实机验收的唯一操作顺序。
...
- Soft Reset 前置审查发现旧脚本在用户确认前就删除 `auth.lock`，并可能按进程名终止其他 Portable 实例的 Gateway。当前修复把停止 Gateway 和全部删除动作移到输入 `yes` 之后，只终止命令行属于当前规范化 Portable 根目录的 Gateway，并明确保留 `data/`、`knowledge/` 和 `logs/`；新增复制到临时夹具中的取消/确认行为测试及非 Portable 根目录拒绝测试。GitHub Actions run `33945993164` 已在 Windows PowerShell 5.1/7 全部通过，下一步为 E 盘部署和实机行为测试。
...
最小初始化器、P0-09 核心 Runtime、幂等启动、10 份组件回执无重装迁移、checkout 清洁化、损坏回执单步重建、Playwright 独立日志、损坏缓存恢复和受控中断续跑均已通过。Soft Reset 的确认/当前实例进程范围修复也已通过 Windows CI；当前第一优先级是部署到 E 盘源实例和脱敏沙箱、运行临时夹具行为测试，然后只在沙箱完成数据保留实测，不执行 Full Reset。GitHub Git 通道恢复后，再续验 P0-07 官方更新证据链。
